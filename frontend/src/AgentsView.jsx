...
const AGENTS = [
  { slug: "swap", name: "REALM Swap Agent", icon: "💱", color: "#8b5cf6", cat: "DEFI",
    ...
    fallback: (q, d) => { const data = d?.data || {}; const ql = q.toLowerCase(); if (ql.match(/pre[cç]o|price|valor|quanto/)) return "REALM: $" + Number(data.price_usd||0).toFixed(6) + " USD\nWETH: $" + Number(data.weth_price_usd||0).toFixed(2); if (ql.match(/swap|trocar|converter/)) { const w=data.weth_price_usd||0, p=data.price_usd||0; return p>0&&w>0 ? "1 WETH = " + Math.round(w/p).toLocaleString() + " REALM\nSwap on Aerodrome (Base)." : "Price data unavailable."; } return "REALM: $" + Number(data.price_usd||0).toFixed(6) + " | WETH: $" + Number(data.weth_price_usd||0).toFixed(2) + "\nAsk about: price, swap quotes, or signals."; },
  ...
  { slug: "rebalancer", name: "Portfolio Rebalancer", icon: "📊", color: "#06b6d4", cat: "DEFI",
    ...
    fallback: (q, d) => { const data = d?.data || {}; const pools = data.pools || []; const ql = q.toLowerCase(); if (ql.match(/best|melhor|top|yield|apy/)) return pools[0] ? "Top pool: " + pools[0].protocol + " " + pools[0].pool + " at " + pools[0].apy + "% APY (" + pools[0].risk + " risk)" : "No pools available."; return "Tracking " + (data.total_pools||0) + " Base pools. Ask about: best yields, safe pools, or risk."; },
  ...
  { slug: "governance", name: "DAO Governance", icon: "🏛️", color: "#f59e0b", cat: "DAO",
    ...
    fallback: (q, d) => { const data = d?.data || {}; const t = data.treasury || {}; const ql = q.toLowerCase(); if (ql.match(/treasury|tesour|saldo/)) return "Treasury: " + (t.eth||0).toFixed(4) + " ETH + " + Number(t.realm||0).toLocaleString() + " REALM"; if (ql.match(/contract|contrato/)) return "Registry: " + (data.contracts?.registry||"") + "\nRouter: " + (data.contracts?.router||""); return "RealmDAO: " + Number(t.realm||0).toLocaleString() + " REALM in treasury. Ask about: treasury, proposals, or contracts."; },
  ...
  { slug: "whale", name: "Whale Intelligence", icon: "🐋", color: "#10b981", cat: "ANALYTICS",
    ...
    fallback: (q, d) => { const data = d?.data || {}; const tx = data.transfers || []; const ql = q.toLowerCase(); if (ql.match(/whale|transfer|movimento/)) return tx.length ? data.count + " whale transfers in " + data.period + ".\nLargest: " + tx[0]?.amount + " REALM" : "No whale transfers detected."; return "Whale Intelligence: " + (data.count||0) + " transfers in " + (data.period||"24h") + ". Ask about: whales, exchange flows, or transfers."; },
...
function ApiDataModal({ data, agent }) {
  ...
  return (<div className="space-y-4"><div className="grid grid-cols-2 gap-3"><div className="bg-gray-800/50 rounded-xl p-3"><div className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Agent</div><div className="text-white text-sm">{typeof data.agent==="string"?data.agent:(data.agent?.name||agent.name)}</div></div><div className="bg-gray-800/50 rounded-xl p-3"><div className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Tier</div><div className="flex items-center gap-1.5"><span className={"w-2 h-2 rounded-full "+(data.tier==="staker"?"bg-yellow-500":data.tier==="holder"||data.tier==="realm"?"bg-purple-500":"bg-green-500")}/><span className="text-white text-sm capitalize">{typeof data.tier==="string"?data.tier:"free"}</span></div></div></div>{entries.length>0&&(<div><div className="text-[10px] text-gray-500 uppercase tracking-wider mb-2">Live Data ({entries.length} fields)</div><div className="space-y-1.5">{entries.map(([k,v])=>(<div key={k} className="flex items-center justify-between bg-gray-800/50 rounded-lg px-3 py-2"><span className="text-gray-400 text-xs font-mono">{k}</span><span className="text-white text-xs font-mono text-right max-w-[60%] truncate">{typeof v==="object"?JSON.stringify(v).substring(0,60):String(v)}</span></div>))}</div></div>)}</div>);
}
...
function ChatModal({ agent, apiData }) {
  ...
  async function send() {
    ...
    setLoading(true);
    try {
      // Try API chat endpoint (Workers AI)
  ...
  return (<div className="flex flex-col h-[60vh]">
    <div className="flex-1 overflow-y-auto space-y-3 mb-3 pr-1">
      {messages.map((m,i)=>(<div key={i} className={m.role==="user"?"flex justify-end":"flex justify-start"}><div className={m.role==="user"?"bg-purple-600/30 border border-purple-500/30 rounded-2xl rounded-br-md px-4 py-2 max-w-[85%]":"bg-gray-800/70 border border-gray-700/50 rounded-2xl rounded-bl-md px-4 py-2 max-w-[85%]"}>{m.role==="agent"&&<div className="text-[10px] font-semibold mb-1" style={{color:agent.color}}>{agent.icon} {agent.name}</div>}<pre className="text-sm text-gray-200 whitespace-pre-wrap font-sans">{m.text}</pre></div></div>))}
      {loading&&(<div className="flex justify-start"><div className="bg-gray-800/70 border border-gray-700/50 rounded-2xl px-4 py-3"><div className="flex gap-1"><div className="w-2 h-2 rounded-full bg-gray-500 animate-bounce" style={{animationDelay:"0ms"}}/><div className="w-2 h-2 rounded-full bg-gray-500 animate-bounce" style={{animationDelay:"150ms"}}/><div className="w-2 h-2 rounded-full bg-gray-500 animate-bounce" style={{animationDelay:"300ms"}}/></div></div></div>)}
      <div ref={bottomRef}/>
    </div>
    {messages.length<=1&&(<div className="flex flex-wrap gap-1.5 mb-3">{agent.suggestions.map((s,i)=>(<button key={i} onClick={()=>setInput(s)} className="text-[11px] px-3 py-1.5 rounded-full bg-gray-800 border border-gray-700 text-gray-400 hover:text-white hover:border-gray-500 transition-colors cursor-pointer">{s}</button>))}</div>)}
...
export default function AgentsView() {
  ...
  useEffect(() => {
    async function load() {
      ...
      await Promise.all(AGENTS.map(async(a)=>{
        try { const res=await fetch(API+"/agents/"+a.slug+"/api/data"); r[a.slug]=await res.json(); } catch(e){r[a.slug]={error:e.message};}
      }));
  ...
  async function openModal(agent, type) {
    ...
    try {
      ...
      if(type==="mcp"){const res=await fetch(url); setModalData(await res.json());}
      else {const res=await fetch(url); setModalData(await res.json());}
  ...
  return (<div className="max-w-6xl mx-auto">
    <div className="text-center mb-8">
      <div className="inline-flex items-center gap-2 bg-purple-500/10 border border-purple-500/30 px-4 py-1 rounded-full text-xs text-purple-400 mb-3"><span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"/>ERC-8004 Compliant • Workers AI</div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {AGENTS.map((agent)=>{const d=data[agent.slug]||{};const st=agent.stats(d);const ok=!d.error;return(
        <div key={agent.slug} className="bg-gray-900/80 border border-gray-800 rounded-2xl overflow-hidden hover:border-purple-500/40 transition-all hover:-translate-y-1">
          ...
          <div className="grid grid-cols-2 border-t border-gray-800/50">{st.map((s,i)=>(<div key={i} className={"p-3 "+(i<2?"border-b border-gray-800/30 ":"")+(i%2===0?"border-r border-gray-800/30":"")}><div className="text-[9px] text-gray-600 uppercase tracking-wider">{s.l}</div><div className="text-sm font-bold text-white mt-0.5">{s.v}</div></div>))}</div>
    ...
    {modal&&(<Modal open={true} onClose={closeModal} title={modal.type==="chat"?"🤖 Chat with "+modal.agent.name:modal.type==="card"?modal.agent.name+" — Agent Card":modal.type==="api"?modal.agent.name+" — Live Data":modal.agent.name+" — MCP Tools"} icon={modal.agent.icon} color={modal.agent.color}>
      {modal.type==="chat"?(<ChatModal agent={modal.agent} apiData={data[modal.agent.slug]}/>):modalLoading?(<div className="text-center py-8"><div className="inline-block w-6 h-6 border-3 border-purple-500/20 border-t-purple-500 rounded-full animate-spin mb-3"/><p className="text-gray-500 text-sm">Fetching data...</p></div>):modalData?.error?(<div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4"><p className="text-red-400 text-sm">Error: {safeStr(modalData.error)}</p></div>):modal.type==="card"?(<AgentCardModal data={modalData} agent={modal.agent}/>):modal.type==="api"?(<ApiDataModal data={modalData} agent={modal.agent}/>):(<McpModal data={modalData} agent={modal.agent}/>)}
    </Modal>)}
  </div>);
}
